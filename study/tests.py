...
class StudyFormTest(TestCase):
    ...
    # タグを2つ選択した場合、StudyFormのバリデーションを通過することを確認
    ...
    # タグを3つ選択した場合、StudyFormのバリデーションエラーになることを確認
...
class StudyModelTest(TestCase):
    ...
    # 同じタグを複数回追加しても、Studyとの関連が重複しないことを確認
    def test_duplicate_tag_relationship_is_not_created(self):
            ...
            self.assertEqual(study.tags.count(), 1)


class TagModelTest(TestCase):

    # 同じ名前のタグを複数作成できないことを確認
    def test_duplicate_tag_name_is_not_allowed(self):
...
class StudyViewTest(TestCase):
    ...
    # DashboardからStudyを作成したとき、ログインユーザーに正しく紐づくことを確認
